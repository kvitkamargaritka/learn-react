...
var ItemList = React.createClass({
  render: function() {
    var dataInfo = this.props.arr.map(function(el) {
      return (
        <Item comments={el.comments} imgSrc={el.imgSrc} name={el.name} memory={el.memory} hdd={el.hdd} description={el.description} key={el.id}/>
      );
    });
  ...
  render: function() {
    return (
      <div className="item__btn">
        <button className="btn btn_green" >Купить</button>
      </div>
    );
  }
});

var BtnDis = React.createClass ({
  render: function() {
    return (
      <div className="item__btn">
        <button className="btn btn_white">Сообщите, когда появится</button>
      </div>
    );
  }
});
...
//  render: function() {
//    var comment = data;
//    var commentInfo = comment.map(function(el) {
//      return (
//
//      );
//    });
